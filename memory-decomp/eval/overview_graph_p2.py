...
def plot_side_by_side_scatter(data_list1, data_list2, file_name, title1, title2, big_title, figsize, colors):
    ...
    all_y_values = []
    for _, data_dict in data_list1 + data_list2:
        all_y_values.extend(data_dict.values())
    
    # Create y-ticks from unique y-values and sort them
    unique_y_values = sorted(set(all_y_values))
    
    # Map y-values to their indices in the sorted unique_y_values list
    y_value_to_index = {value: idx for idx, value in enumerate(unique_y_values)}
    ...
    def plot_data(ax, data_list, color_list, include_legend=False):
        for (name, data_dict), color in zip(data_list, color_list):
            ...
            # Map values to their indices in the unique_y_values list
            y_indices = [y_value_to_index.get(v, -1) for v in values]
            
            # Filter out indices that are not valid
            filtered_indices = [i for i in indices if 0 <= i < len(values)]
            filtered_y_indices = [y_indices[i] for i in range(len(values)) if i in filtered_indices]
            
            # Plot data
            ax.scatter(filtered_indices, filtered_y_indices, color=color, edgecolor='black', label=name)
    ...
    ax1.set_yticks(range(len(unique_y_values)))
    ax1.set_yticklabels(unique_y_values)
    ...
    ax2.set_yticks(range(len(unique_y_values)))
    ax2.set_yticklabels(unique_y_values)
...
plot_side_by_side_scatter(benchmarks, predictions, "eval/overview_p2.png", "Benchmark", "Predictions", "Register Aggregation", (30, 7), colors)

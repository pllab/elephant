...
def test_openram_sram(height_log2, width, num_read_ports = 1, num_write_ports = 1):
    pyrtl.reset_working_block()

    print(f"test {num_read_ports}r{num_write_ports}w OpenRAM SRAM h{2 ** height_log2}_w{width}")
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    ...
    rdatas = []
    for i in range(num_read_ports):
        raddrs.append(pyrtl.Input(addr_width, f"raddr{i}"))
        rdatas.append(pyrtl.Output(val_width, f"rdata{i}"))

    waddrs = []
    wdatas = []
    w_ens = []
    for i in range(num_write_ports):
        waddrs.append(pyrtl.Input(addr_width, f"waddr{i}"))
        wdatas.append(pyrtl.Input(val_width, f"wdata{i}"))
        w_ens.append(pyrtl.Input(1, f"w_en{i}"))
...
if __name__ == '__main__':
    for height_log2 in range(4,8):
        for width in range(16,17):
            ...
            with open('openram_h{0}_w{1}_{2}r{3}w.py'.format(2 ** height_log2, width, num_read_ports, num_write_ports), 'w') as f:
                f.write(test_openram_sram(height_log2, width, num_read_ports=1, num_write_ports=1))
